...
if ($_GET['deleteEvent'] != "")
{
	...
	$query = "DELETE FROM ProgramPerson WHERE ProgramID=" . $delEvent;
	$result = doSQLQuery($query);
}


// Nothing chosen, so shown the new event form
if ($_GET['day'] < 1 && $_POST['date'] == "" && $_POST['title'] == "" && $_GET['eventId'] == "") {
?>
...
<?php
		$query = "SELECT ProgramPerson.PersonID,Person.Fornavn,Person.Efternavn,ProgramPerson.RolleID FROM ProgramPerson INNER JOIN Person ON ProgramPerson.PersonID=Person.PersonId WHERE ProgramPerson.ProgramID=".$eventId;
		$result = doSQLQuery($query);
		$personCount = 0;
		while ($line = db_fetch_array($result)) {
			if ($personCount % 2 == 1) {
...
?>
	<tr<?php echo $color; ?>>
		<td><div id="<?php echo $line["PersonID"]; ?>"><?php echo $line["Fornavn"]." ".$line["Eftenavn"]; ?></div></td>
...
<?php
			...
			$abilityQuery = "SELECT Rolle.RolleId,Rolle.Navn FROM Rolle INNER JOIN PersonRolle ON Rolle.RolleID=PersonRolle.RolleID WHERE PersonRolle.PersonID=".$personid.";";
